...
class TeamFragment : Fragment(R.layout.fragment_general_main_team), TeamAdapter.OnWorkshopAreaClickListener {
    ...
    // TeamFragment implementa TeamAdapter.OnWorkshopAreaClickListener per gestire i click sulle aree di lavoro del team
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mediatorLiveData.addSource(teamViewModel.listMembers) { listMembers -> // Aggiunge listMembers come sorgente di dati per mediatorLiveData
            // viene creata una coppia di liste di membri del team e dipartimenti e viene assegnata a mediatorLiveData.value
            mediatorLiveData.value = Pair(listMembers, teamViewModel.listDepartments.value)
        }
        mediatorLiveData.addSource(teamViewModel.listDepartments) { listDepartments -> // Aggiunge listDepartments come sorgente di dati per mediatorLiveData
            // viene creata una coppia di liste di membri del team e dipartimenti e viene assegnata a mediatorLiveData.value
            mediatorLiveData.value = Pair(teamViewModel.listMembers.value, listDepartments)
        }

       // Viene aggiunto un observer a mediatorLiveData per ricevere gli aggiornamenti dei valori di listMembers e listDepartments
        mediatorLiveData.observe(viewLifecycleOwner) { (listMembers, listDepartments) ->
            ...
            if (listMembers != null && listDepartments != null) {
                // Crea un oggetto LinearLayoutManager che verrà utilizzato come layout manager per la RecyclerView
                val layoutManager = LinearLayoutManager(context)
                recyclerView = view.findViewById(R.id.membersWorkshopAreasRecyclerView) // riferimento all'elemento RecyclerView
                recyclerView.layoutManager = layoutManager // Imposta il layout manager alla recyclerView
                // Crea un'istanza di TeamAdapter passando il teamViewModel e l'implementazione di OnWorkshopAreaClickListener
    ...
    override fun onWorkshopAreaClick(workshopAreaClicked: String) {
        val department: String = workshopAreaClicked // Ottiene l'area di lavoro del team selezionata dal parametro workshopAreaClicked
        adapter.switchingVisibility(department) // Chiama il metodo switchingVisibility sull'adapter per gestire la visibilità degli elementi
    }
}